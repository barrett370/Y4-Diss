...
The final set of routes is finally filtered by the predicate $\texttt{infeasible-distance}(i) = 0, \forall i \in P $
...
\begin{equation}
\mathcal{X} = \left[p_{1_{x}}, p_{n_{x}}\right]
\end{equation}
...
\begin{equation}
  \mathcal{Y} = \left[ - 1.3 \cdot \frac{b_{1}(x_{1}) + b_{2}(x_{2})}{2}, 1.3 \cdot \frac{b_{2}(x_{1}) + b_{2}(x_{2})}{2} \right]
\end{equation}
...
A pair standard deviations as previously outlined in the Background section is defined as:
...
\end{figure}\feedback{Is this figure clear enough? Should I move it to appendix?}
