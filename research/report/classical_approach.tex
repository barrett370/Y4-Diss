...
\textbf{Note:} this format of figure will be used throughout this report and shows a top-down view of the road space, the $y$ axis corresponds to the road width and the $x$ axis to the road length.
...
Each of the operators outlined in this section are then applied in the order shown in Algorithm~\ref{alg:GenericGA}, until the termination criteria are met. In this case my only termination criteria was the maximum number of generations had taken place.
...
\begin{figure}
  \centering
  \begin{tikzpicture}
    \node[] (in) {$\{ a_{1}, a_{2}, \ldots, a_{n} \} $};
  \node[draw, rectangle, right = 1.5cm of in] (V) {$\texttt{PCCGA}$};

  \node[draw, rectangle, above right= of V] (ga1) {$\texttt{PCCGA}(a_{1})$};
  \node[draw, rectangle, below = of ga1] (ga2) {$\texttt{PCCGA}(a_{2})$};
  \node[below = of ga2] (gael) {$\vdots$};
  \node[draw, rectangle, below = of gael] (gan) {$\texttt{PCCGA}(a_{n})$};
...
Once these groupings have been constructed, the next stage is to plan the low-level curves representing the paths each agent must take through each road segment, avoiding any agents it shares a given road segment with.

\subsection{Multi-agent route generation}

This process is a wrapper around the previously implemented parallel planner described in Section~\ref{sec:maa}.
...
My path groupings algorithm as described in Algorithm~\ref{alg:pathgrouping} is then applied to these \textit{abstract plans}.
