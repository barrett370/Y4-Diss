...
\begin{algorithm}
  \caption{Path Grouping}\label{alg:pathgrouping}
  \KwIn{
  \begin{itemize}
    \item a road network graph $\mathcal{R} = (V,E)$
    \item a macro-path for each agent, $\mathcal{V}$.
  \end{itemize}
}
...
\For{each agent's macro-path, $i \in \mathcal{V}$}{
    ...
    \For{each edge, $e \in E$ in the macro-path}{
      ...
      \For{each other agent's macro-path, $o \in \mathcal{V}\backslash i$}{
